...
const Header = (props) => {
  ...
  const fontMenuChnageHandler = () => {
    setFontMenu(!fontMenu);
  };

  const monoHandler = (event) => {
    event.stopPropagation();
    setFontMenu(fontMenu);
    props.onMonoClick();
  };

  const serifHandler = (event) => {
    event.stopPropagation();
    setFontMenu(fontMenu);
    props.onSerifClick();
  };

  const sansSerifHandler = (event) => {
    event.stopPropagation();
    setFontMenu(fontMenu);
          ...
          >
            ...
            {fontMenu && (
              <div className=" sm:absolute sm:right-32 sm:top-16 md:right-36 md:top-24 xl:mr-[310px]  shadow-[0px_5px_30px_#A445ED;] rounded-2xl bg-[#1F1F1F] flex flex-col gap-2 pt-5 pr-14 pb-6 pl-6 ">
                ...
                <div
                  className="font-bold text-white text-[14px] leading-6  "
                  onClick={serifHandler}
                >
                  Serif
                </div>
                <div
                  className="font-bold text-white text-[14px] leading-6  "
                  onClick={sansSerifHandler}
                >
                  Sans Serif
                </div>
              </div>
